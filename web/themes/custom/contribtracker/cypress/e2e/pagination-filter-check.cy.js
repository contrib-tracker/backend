...
describe('Pagination Handling and Validation', () => {
    ...
    it('should filter results with selected contribution type on each page', () => {
      ...
      // Wait for potential loading state before results show up
  ...
  function filterAssertion() {
    ...
    cy.get('body').then(($body) => {
      if ($body.find('.no-results-message').length > 0) {
        // If no results, log a message and stop the test for this page
        cy.log('No results found for the selected filter');
      } else {
  ...
  function clickNextButton() {
    cy.get('body').then((pagination) => {
      ...
      if (pagination.find('.pager__item--next').length > 0) {
        // Handle potential loading state before navigating to the next page
        cy.get('.loading-indicator', { timeout: 10000 }).should('not.exist');
        ...
        // Assert the filtered results on the next page
        filterAssertion();
